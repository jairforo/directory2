...
aitRenderDeleteCachesThemeOptionControl(){?>
		...
		<div class="ait-opt">
			<div class="ait-opt-wrapper" style="background:none;">
				<input type="button" id="ait-delete-cache-theme-btn" class="ait-opt-btn" value="<?php _e('Empty theme cache','ait-admin')?>">
				<input type="button" id="ait-delete-cache-image-btn" class="ait-opt-btn" value="<?php _e('Empty image (WPThumb) cache','ait-admin')?>">
			</div>
		</div>
		<script>
			jQuery(function($){
...
false;$post=get_post();$slug='';if($post
...
is_callable($this->params->saveCallback)){call_user_func_array($this->params->saveCallback,array($postId,$post,$this,$data));}else{$realPostId=isset($_POST['post_ID'])?$_POST['post_ID']:null;if(defined('DOING_AUTOSAVE')AND
...
AitUpgrader{protected$skeletonVersionOptionKey;protected$themeVersionOptionKey;protected$errors=array();static
function
run(){add_action('init',array(__CLASS__,'onInit'));}static
function
onInit(){if(AitUtils::isAjax()or
defined('IFRAME_REQUEST'))return;$upgrader=new
self;$upgrader->checkVersionAndMaybeDoUpgrade();}function
__construct(){if(get_option("_ait_".AIT_CURRENT_THEME."_skeleton_version")===false){$this->skeletonVersionOptionKey="_ait_skeleton_version";$this->themeVersionOptionKey="_ait_theme_version";}else{$this->skeletonVersionOptionKey="_ait_".AIT_CURRENT_THEME."_skeleton_version";$this->themeVersionOptionKey="_ait_".AIT_CURRENT_THEME."_theme_version";}}function
checkVersionAndMaybeDoUpgrade(){if(version_compare($this->getSkeletonVersion(),AIT_SKELETON_VERSION,'!=')){$this->skeletonUpgrade();}if(version_compare($this->getThemeVersion(),AIT_THEME_VERSION,'!=')){$this->themeUpgrade();}if(empty($this->errors)){if(version_compare($this->getSkeletonVersion(),AIT_SKELETON_VERSION,'!=')){$this->updateSkeletonVersion(AIT_SKELETON_VERSION);AitCache::clean();}if(version_compare($this->getThemeVersion(),AIT_THEME_VERSION,'!=')){$this->updateThemeVersion(AIT_THEME_VERSION);AitCache::clean();}}}function
skeletonUpgrade(){if(version_compare($this->getSkeletonVersion(),'2.1.4','<')){$upgrade=new
AitSkeletonUpgrade21;$this->errors=$upgrade->execute();}if(version_compare($this->getSkeletonVersion(),'2.2.3','<')){$upgrade=new
AitSkeletonUpgrade223;$errors=$upgrade->execute();$this->errors=array_merge($this->errors,$errors);$this->skeletonVersionOptionKey="_ait_".AIT_CURRENT_THEME."_skeleton_version";$this->themeVersionOptionKey="_ait_".AIT_CURRENT_THEME."_theme_version";}if(version_compare($this->getSkeletonVersion(),'2.8.12','<')){$upgrade=new
AitSkeletonUpgrade2812;$this->errors=$upgrade->execute();}do_action('ait-skeleton-upgrade',$this);$this->addAdminNotices();}function
themeUpgrade(){do_action('ait-theme-upgrade',$this);$this->addAdminNotices();}function
...
function
addAdminNotices(){if(!empty($this->errors)){add_action('admin_notices',array($this,'adminNotices'));}}function
adminNotices(){echo'<div class="error">';foreach($this->errors
as$error){echo"<p>";echo
esc_html($error);echo"</p>";}echo'</div>';}function
getSkeletonVersion(){return
get_option($this->skeletonVersionOptionKey,'0.0.1');}function
getThemeVersion(){return
get_option($this->themeVersionOptionKey,'0.0.1');}function
updateSkeletonVersion($newVersion){update_option($this->skeletonVersionOptionKey,$newVersion);}function
updateThemeVersion($newVersion){update_option($this->themeVersionOptionKey,$newVersion);}}class
AitUtils{function
__construct(){throw
new
LogicException(__CLASS__.' is a static class. Can not be instantiate.');}static
...
as$i=>$el){if(!$em->getPrototype($el->getId())and!$em->isElementSidebarsBoundary($el->getId()))continue;if($el->isDisabled())continue;if($em->isElementSidebarsBoundary($el->getId())and(!self::hasSidebar(__CLASS__,'left')&&!self::hasSidebar(__CLASS__,'right')))continue;if(!$el->sortable){$return->unsortable[$el->getId()]=$el;}elseif(isset($options[$i][$el->getId()]['@columns-element-index'])and$options[$i][$el->getId()]['@columns-element-index']!=''and
...
function
prepareDefaultValue($optionControlDefinition){$d=array('color'=>'','opacity'=>1,'image'=>'','repeat'=>'','position'=>'','scroll'=>'');$d=array_merge($d,$optionControlDefinition['default']);$opacity=isset($d['opacity'])?floatval($d['opacity']):1;if($opacity>1)$opacity/=100;if(AitUtils::startsWith($d['color'],'#')and$opacity!=1){$r=0;$g=0;$b=0;extract(AitUtils::hex2rgb($d['color']));$rgba=sprintf("rgba(%s, %s, %s, %s)",$r,$g,$b,$opacity);$d['color']=$rgba;}if(isset($d['image'])and!AitUtils::isExtUrl($d['image'])and$d['image']!=''){$d['image']=aitPaths()->url->theme.$d['image'];}return$d;}function
...
as$i=>$clonedOptionsControlsSection){$id="{$containerId}-{$i}-pregenerated";$class="ait-clone-item ait-pregenerated-clone-item";$firstTextInputValue=$this->getClonedOptionsControlsSectionLabel($clonedOptionsControlsSection);?>

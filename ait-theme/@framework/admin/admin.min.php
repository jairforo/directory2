...
and$post->post_type=='page'and$s->id=='page'){printf('<a href="#" id="ait-goto-page-builder-button" class="button button-primary" data-ait-empty-title-note="%s">%s</a>',__('Please enter title of the page','ait-admin'),__('Save and Open in Page Builder','ait-admin'));}}static
function
modifyPageRowActions(){add_filter('page_row_actions',array(__CLASS__,'addPageBuilderLinkToPageRowActions'),10,2);}static
function
addPageBuilderLinkToPageRowActions($actions,$page){$args=array('page'=>'pages-options','oid'=>'_page_'.$page->ID);if($page->post_status!='auto-draft'){if(get_option('show_on_front')=='page'){if($b=get_option('page_for_posts')){$blog=(int)$b;if($page->ID==$blog)$args['oid']="_blog";}}$title=__('Page Builder','ait-admin');$args['oidnonce']=AitUtils::nonce('oidnonce');$url=esc_url(AitUtils::adminPageUrl($args));$link="<a href=\"$url\">$title</a>";$actions['page_builder']=$link;}return$actions;}static
function
...
AitAutomaticPluginActivation{static
function
run(){if(class_exists('TGM_Bulk_Installer',false)){$plugins=self::getPrepackedPluginsPaths();$installer=new
TGM_Bulk_Installer();$installer->init();$installer->strings['unpack_package']='';$installer->strings['installing_package']='';$creds=request_filesystem_credentials('','direct',false,false);WP_Filesystem($creds);foreach($plugins
as$pluginPackageFile){self::installAndActivatePlugin($pluginPackageFile,$installer);}}}protected
static
function
installAndActivatePlugin($pluginPackageFile,$installer){$workingDir=$installer->unpack_package($pluginPackageFile,false);if(is_wp_error($workingDir)){return$workingDir;}$result=$installer->install_package(array('source'=>$workingDir,'destination'=>WP_PLUGIN_DIR,'clear_destination'=>false,'clear_working'=>true,'hook_extra'=>array('plugin'=>$pluginPackageFile)));wp_cache_flush();if(is_wp_error($result)){if($result->get_error_code()=='folder_exists'){$pluginsTargetPath=$result->get_error_data('folder_exists');$slug=basename($pluginsTargetPath);$pluginData=get_plugins('/'.$slug);$pluginFile=array_keys($pluginData);$pluginInfo=$slug.'/'.$pluginFile[0];}}else{$pluginInfo=$installer->plugin_info();}$activate=activate_plugin($pluginInfo);if(is_wp_error($activate)){}else{}wp_cache_flush();}protected
static
function
...
self::$isExportingDemoContent){$where=" WHERE (post_status != 'auto-draft' and post_status !=  'trash') and post_type != 'revision'";}$result=$wpdb->get_results("SELECT * FROM {$wpdb->$table}{$where}",ARRAY_A);$dump[$table]=$result?$result:array();}$customTables=apply_filters('ait-backup-content-custom-tables',array(),self::$isExportingDemoContent);if(!empty($customTables)and
...
as$mime){$exts=array_merge($exts,explode('|',$mime));}array_walk($exts,array(__CLASS__,'buildExt'));$foundFiles=NFinder::findFiles($exts)->filter(array(__CLASS__,'filterNonMediaFiles'))->from($uploadsDir);$i=-1;foreach($foundFiles
as$pathname=>$file){$i++;$dump['@attachments'][$i]['url']="$uploadsUrl/".self::getRelativePathname($file->getPathname());$dump['@attachments'][$i]['basename']=self::getRelativePathname($file->getPathname());if(self::$isExportingDemoContent){$size=@getimagesize($file->getRealPath());if($size===false)continue;$ext=$file->getExtension();$url=self::DUMMY_IMG_URL."?w={$size[0]}&h={$size[1]}&ext={$ext}";$dump['@attachments'][$i]['url']=$url;}}}protected
static
function
...
$this->renderHeaderTitle();$this->renderHeaderTools();?>
		...
		<div class="ait-options-page" data-unsaved-changes-message="<?php _e("Changes have been made.",'ait-admin');?>">
...
$elementsControlsRenderer=AitElementsControlsRenderer::create(array('configType'=>'elements','adminPageSlug'=>'pages-options','oid'=>$this->oid,'fullConfig'=>aitConfig()->getFullConfig('elements'),'defaults'=>aitConfig()->getDefaults('elements'),'options'=>aitOptions()->getOptionsByType('elements',isset($this->importFrom)?$this->importFrom:$this->oid)),'AitElementsControlsRenderer');?>
					...
					<div id="ait-used-elements" class="ait-elements">
						...
						<h2 class="ait-elements-group"><?php _e('Sticked unsortable elements','ait-admin')?></h2>
						<p class="ait-elements-placeholder-note"><?php _e('You can only enable or disable these elements','ait-admin')?></p>
						<div id="ait-used-elements-unsortable" class="ait-used-elements">
						<?php
$elementsControlsRenderer->renderUsedUnsortableElements();?>
						</div>

						<h2 class="ait-elements-group"><?php _e('Sortable elements','ait-admin')?></h2>
						<p class="ait-elements-placeholder-note"><?php _e('Drag&Drop here elements from the right hand side','ait-admin')?></p>
				...
				<li class="ait-page-import">
					...
					<div id="ait-page-options-import-selection-popup" style="display:none;">
						<div id="ait-page-options-import-selection-content-wrapper">
							<h1><?php _e('Import','ait-admin');?></h1>
							<?php $this->renderPagesDropdown('page-options-import-selection',__('Select a page from which to import options','ait-admin'),'',true);?>
...
$nonce=AitUtils::nonce('delete-local-options');printf('<a href="%s" data-ait-delete-local-options=\'%s\'>%s</a>',esc_url(add_query_arg('oid',$this->oid)),json_encode(array('oid'=>$this->oid,'nonce'=>$nonce)),_x('Delete','options','ait-admin'));?>
				...
				<li class="ait-page-new">
					<a href="<?php echo
admin_url('post-new.php?post_type=page')?>" title="<?php _e('Add New Page','ait-admin')?>">
...
addViewAndEditLinksToAdminBar($wp_admin_bar){global$post;if(!$post)return;if($post->post_type=='page'){$wp_admin_bar->add_node(array('id'=>'view-page','title'=>__('View Page','ait-admin'),'href'=>get_permalink($post->ID)));$wp_admin_bar->add_node(array('id'=>'edit-page','title'=>__('Edit Page','ait-admin'),'href'=>get_edit_post_link($post->ID)));}}}class
...
echo
aitDataAttr('reset-options',array('confirm'=>__('Are you sure you want to reset "Theme Options"?','ait-admin'),'nonce'=>AitUtils::nonce("reset-{$this->pageSlug}"),'what'=>$this->pageSlug));?>
						>
						...
						<li><a href="#" class="ait-reset-options"
							<?php echo
aitDataAttr('reset-options',array('confirm'=>__('Are you sure you want to reset all options?','ait-admin'),'nonce'=>AitUtils::nonce("reset-all-options"),'what'=>'all'));?>
						>
